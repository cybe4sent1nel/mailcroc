...
import styles from './AILogo.module.css';

interface AILogoProps {
    className?: string;
    size?: number;
    hover?: boolean; // If true, applies hover effect
    loading?: boolean; // If true, spins
    color?: string;
    fill?: string;
    stroke?: string;
}
...
export const AILogo: React.FC<AILogoProps> = ({
    ...
    // If color is provided, use it for both fill and stroke for a solid look
    ...
    return (
        <svg
            className={`
                ${styles.logo} 
                ${hover && !loading ? styles.hoverable : ''} 
                ${loading ? styles.spin : ''} 
                ${className || ''}
            `}
            width={size}
