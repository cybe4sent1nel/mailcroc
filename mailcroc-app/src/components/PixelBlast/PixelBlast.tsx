...
void main(){
  ...
  if (uGap > 0.0) {
    vec2 pUV2 = fract(gl_FragCoord.xy / uPixelSize);
    float gapNorm = uGap / uPixelSize;
    // Keep 1.0 - gapNorm as the filled region centered
    // Min bound: gapNorm/2
    // Max bound: 1.0 - gapNorm/2
    vec2 border = step(vec2(gapNorm * 0.5), pUV2) * step(pUV2, vec2(1.0 - gapNorm * 0.5));
    M *= border.x * border.y;
  }
