...
const StatusItem = ({ name, icon: Icon, data }: { name: string, icon: React.ElementType, data: any }) => (
                ...
                <div className={styles.metricsWrapper}>
                    <div className={styles.systemGrid}>
                        <div className={styles.gridSection}>
                            ...
                            <StatusItem name="Mail Delivery" icon={ShieldCheck} data={(statusData as any)?.systems?.smtp} />
                            <StatusItem name="Core API" icon={Zap} data={(statusData as any)?.systems?.api} />
                            <StatusItem name="Database Cluster" icon={Activity} data={(statusData as any)?.systems?.db} />
                        </div>

                        <div className={styles.gridSection}>
                            <h3 className={styles.sectionTitle}>Advanced Services</h3>
                            <StatusItem name="Secure Portal" icon={Lock} data={(statusData as any)?.systems?.secure} />
                            <StatusItem name="AI Analysis" icon={Bot} data={(statusData as any)?.systems?.ai} />
                            <StatusItem name="Voice & Audio" icon={Volume2} data={(statusData as any)?.systems?.voice} />
                        </div>

                        <div className={styles.gridSection}>
                            <h3 className={styles.sectionTitle}>Identity & Trust</h3>
                            <StatusItem name="Stealth Verification" icon={Fingerprint} data={(statusData as any)?.systems?.identity} />
                        </div>
                    </div>
